...
void media_set_raw_mouse_motion_enabled(int enabled) {
    ...
    if (!raw_mouse_motion_enabled) {
        WARN("Raw mouse movement is DISABLED by default for WSL support. If your mouse is having sensitivity issues, please uncheck 'Disable Raw Mouse Motion (For WSL)' on launch.");
    }
}
